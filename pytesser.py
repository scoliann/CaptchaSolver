...
def call_tesseract(input_filename, output_filename, currentDir):
	"""Calls external tesseract.exe on input file (restrictions on types),
	outputting output_filename+'txt'"""
	args = [currentDir + tesseract_exe_name, input_filename, output_filename]
	proc = subprocess.Popen(args)
	retcode = proc.wait()
	if retcode!=0:
		errors.check_for_errors()

def image_to_string(im, currentDir = '', cleanup = cleanup_scratch_flag):
	...
	try:
		...
		# Generate a unique random scratch_image_name and scratch_text_name_root
		import random
